...
import {
  Loader, ArrowLeft, Check, Package, ShoppingBag,
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-0 md:gap-8">
            ...
            <div className="p-6 md:p-10 flex flex-col">
              ...
              >
                <motion.div variants={{ hidden: { opacity: 0, y: 10 }, visible: { opacity: 1, y: 0 } }} className="mb-2">
                  <span className="text-blue-600 font-bold text-sm tracking-widest uppercase">
                    CHÍNH HÃNG
                  </span>
                </motion.div>

                <motion.h1 variants={{ hidden: { opacity: 0, y: 10 }, visible: { opacity: 1, y: 0 } }} className="text-3xl md:text-4xl font-bold text-gray-900 mb-4 leading-tight">
                  {product.name}
                </motion.h1>

                {/* Rating giả lập (Tạo độ uy tín) */}
                <motion.div variants={{ hidden: { opacity: 0, y: 10 }, visible: { opacity: 1, y: 0 } }} className="flex items-center gap-2 mb-6">
                ...
                <motion.div variants={{ hidden: { opacity: 0, y: 10 }, visible: { opacity: 1, y: 0 } }} className="flex items-end gap-4 mb-8">
                ...
                {/* Mô tả ngắn gọn */}
                <motion.div variants={{ hidden: { opacity: 0, y: 10 }, visible: { opacity: 1, y: 0 } }} className="prose text-gray-600 mb-8 line-clamp-3">
                  <p>{product.description || "Sản phẩm công nghệ cao cấp, bảo hành chính hãng."}</p>
                </motion.div>

                <motion.div variants={{ hidden: { opacity: 0, y: 10 }, visible: { opacity: 1, y: 0 } }} className="border-t border-b border-gray-100 py-6 mb-8 space-y-6">
                  {/* Bộ chọn số lượng */}
                ...
                {/* KHU VỰC NÚT BẤM */}
                <motion.div variants={{ hidden: { opacity: 0, y: 10 }, visible: { opacity: 1, y: 0 } }} className="flex flex-col sm:flex-row gap-4 w-full mb-8">
                  <button
                    disabled={product.stock === 0}
                    onClick={handleAddToCart}
                    className="flex-1 bg-blue-50 hover:bg-blue-100 text-blue-700 font-bold py-3.5 rounded-xl transition flex items-center justify-center gap-2 border border-blue-200 disabled:opacity-50 disabled:cursor-not-allowed hover:shadow-md active:scale-[0.98]"
                  >
                    <ShoppingBag size={20} />
                    Thêm vào giỏ
                  </button>
                  ...
                  <button
                    ...
                    className="flex-1 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white font-bold py-3.5 rounded-xl transition shadow-lg shadow-blue-500/30 disabled:opacity-50 disabled:cursor-not-allowed hover:shadow-xl hover:-translate-y-0.5 active:scale-[0.98]"
                  >
                    Mua ngay
                  </button>
                </motion.div>
                ...
                <motion.div variants={{ hidden: { opacity: 0 }, visible: { opacity: 1 } }} className="grid grid-cols-2 gap-4 text-sm text-gray-600">
